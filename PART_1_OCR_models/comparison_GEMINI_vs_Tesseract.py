...
# CHANGED for Tesseract
RES_DIR = BASE_DIR / "PART_1_OCR_models" / "MD_tesseract"
...
_ROMAN_TOKEN_RE = re.compile(r"\b[ivxlcdmІЇХХССВВММДДЛЛ]+\b", re.I)
_LEADING_SECTION = re.compile(
    r"^\s*((?:[ivxlcdmІЇХХССВВММДДЛЛ]+|[0-9]+))\.?\s+", re.I
)  # strip leading I./ІІ./1. etc.
...
def write_html(df: pd.DataFrame, out_html: Path, gt_path: Path, res_path: Path):
    ...
    # CHANGED header labels for Tesseract
    html_header = (
        "<tr>"
        "<th>Gemini label</th>"
        "<th>Tesseract label</th>"
        "<th>Gemini numbers</th>"
        "<th>Tesseract numbers</th>"
        "<th>Label sim</th>"
        "<th>Number sim</th>"
        "<th>Overall sim</th>"
        "</tr>"
    ...
    # CHANGED title for Tesseract
    title = "Markdown Comparison Report — Gemini vs Tesseract"
...
def build_index_html(rows: List[Dict[str, Any]], out_path: Path):
    ...
    # CHANGED titles for Tesseract
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<title>Gemini vs Tesseract — Index</title>
<style>
...
<body>
  <div class="topbar">Markdown Comparison — Index (Gemini folder ↔ Tesseract folder)</div>
  <div class="container">
    <table>
      <tr>
        <th>Gemini file</th>
        <th>Tesseract file</th>
        <th>Report</th>
        <th>Avg overall</th>
        <th>Avg label</th>
        <th>Avg numeric</th>
